...
{$IFDEF DEBUG}
  ...
  DeleteFile(Concat(ExtractFileDir(Application.ExeName), '\LLLauncher\lang\en_us.json'));
{$ENDIF}
  var zhcnjson: TJSONObject;
  if not FileExists(Concat(ExtractFileDir(Application.ExeName), '\LLLauncher\lang\zh_cn.json')) then begin
    zhcnjson := TJSONObject.Create
  ...
  //            '所选更新日志：${changelog}', #13#10
      ...
      .AddPair('messagebox_manage.cannot_unzip_modpack.text', '解压整合包失败了！你似乎给整合包上了密码，或者这个整合包不是正确的整合包，请重试！')
      ...
      .AddPair('inputbox_mainform.unlock_check.caption', '解锁码输入')
      .AddPair('inputbox_mainform.unlock_check.text', '您的解锁码原密钥是：${unlock_code}' + #13#10 + '请将该解锁码原密钥复制给作者，然后说明对应的需求，并提供证据以求解锁码解密钥。' + #13#10 + '请输入对应的解锁码解密钥以解锁对应的功能！')
      .AddPair('messagebox_mainform.unlock_check_error.caption', '解锁码解密钥输入错误')
      .AddPair('messagebox_mainform.unlock_check_error.text', '您的解锁码解密钥输入错误啦！请尝试重新输入一次吧！')
      .AddPair('messagebox_mainform.unlock_check_plugin.caption', '插件部分解锁成功')
      .AddPair('messagebox_mainform.unlock_check_plugin.text', '您的解锁码输入成功！现在你已解锁了插件部分！')
      .AddPair('messagebox_plugin.has_not_unlock_plugin.caption', '暂未解锁插件部分')
      .AddPair('messagebox_plugin.has_not_unlock_plugin.text', '该功能为内测专属，您还暂未解锁插件部分，请进入内测群私聊作者获取解锁码后，在菜单栏的【更多 -> 解锁码】输入解锁码解密钥再来吧！')
      .AddPair('inputbox_export.cannot_get_vanilla.caption', '未能读取到原版键')
      .AddPair('inputbox_export.cannot_get_vanilla.text', '该版本未能读取到原版键，该版本很可能是由不知名小启动器下载的。请尝试在下方输入栏输入该版本的原版键以供读取。'#13#10'示例：1.20.4-forge-49.1.0，输入1.20.4即可。'#13#10'输入错误而导致读取不了概不负责。')
      //以下为下载进度列表框
      ...
      .AddPair('downloadlist.modpack.download_modpack_success', '下载整合包已完成，耗时：${download_finish_time}秒')
      //以下为下载进度窗口
      ...
      .AddPair('stringgrid_progress_download_details.caption.filename', '下载文件名')
      .AddPair('stringgrid_progress_download_details.caption.current', '进度')
      //以下为主窗口
      ...
      ;
    zhcnjson := SortedLANGJSON(zhcnjson);
    SetFile(Concat(ExtractFileDir(Application.ExeName), '\LLLauncher\lang\zh_cn.json'), zhcnjson.Format);
  end else begin
    zhcnjson := TJSONObject.ParseJSONValue(GetFile(Concat(ExtractFileDir(Application.ExeName), '\LLLauncher\lang\zh_cn.json'))) as TJSONObject;
  end;
  alllangjson.Add(zhcnjson);
  var enusjson: TJSONObject;
  if not FileExists(Concat(ExtractFileDir(Application.ExeName), '\LLLauncher\lang\en_us.json')) then begin
    enusjson := TJSONObject.Create
  ...
  //            '所选更新日志：${changelog}', #13#10
      ...
      .AddPair('plugin_menu_back.caption', '回退')
      ;
    enusjson := SortedLANGJSON(enusjson);
    SetFile(Concat(ExtractFileDir(Application.ExeName), '\LLLauncher\lang\en_us.json'), enusjson.Format);
  end else begin
    enusjson := TJSONObject.ParseJSONValue(GetFile(Concat(ExtractFileDir(Application.ExeName), '\LLLauncher\lang\en_us.json'))) as TJSONObject;
  end;
