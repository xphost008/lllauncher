...
begin
  ...
  try
    var path := ((MCJSON.GetValue('mc') as TJSONArray)[mselect_mc] as TJSONObject).GetValue('path').Value;
...
begin
  ...
  var selpath := ((MCSelJSON.GetValue('mcsel') as TJSONArray)[mselect_ver] as TJSONObject).GetValue('path').Value;
  var mcpath := ((MCJSON.GetValue('mc') as TJSONArray)[mselect_mc] as TJSONObject).GetValue('path').Value;
  if IsJSONError(selpath) then begin
...
begin
  ...
  try //添加文件列表，并判断游戏文件的规范性，如果规范，则正常输出。
    ...
    form_mainform.label_version_current_path.Caption := GetLanguage('label_version_current_path.caption').Replace('${current_path}', ((MCJSON.GetValue('mc') as TJSONArray)[mselect_mc] as TJSONObject).GetValue('path').Value);
  except
